...
Soil testing is a widely used, though inherently imperfect, diagnostic tool for guiding phosphorus fertilizer recommendations. Soil sampling typically involves collecting multiple cores from the 0 to 15 cm depth and combining them into a single composite sample, often using about 10 to 12 cores per composite ([Lyons et al., 2022](https://doi.org/10.1080/00224561.2002.12457476)). Soil test methods rely on chemical extractants designed to estimate a fraction of total soil phosphorus that is assumed to be proportional to plant-available P. However, the extracted fraction represents only a subset of soil P pools and does not directly measure total P availability to crops. The remaining phosphorus may be bound to soil minerals, associated with organic matter, or otherwise unavailable to plants on agronomically relevant timescales. Moreover, the relationship between extracted P and crop availability can vary substantially across soil types and management histories due to factors such as clay content, mineralogy, pH, and buffering capacity, which influence how phosphorus interacts with the extraction solution and can reduce the consistency of soil test interpretations across environments ([Mallarino et al., 2002](https://doi.org/10.1080/00224561.2002.12457476)).

Once soil test phosphorus (STP) is quantified, the next step is typically to relate STP values to observed crop yield responses to phosphorus fertilization. These correlation-based approaches use yield data from fertilizer response trials to estimate a critical soil test value (CSTV), defined as the STP concentration at which crop yield is no longer expected to increase appreciably with additional phosphorus inputs. Below the CSTV, yield responses to fertilization are more likely, whereas above the CSTV, additional P is generally considered agronomically unnecessary ([McGrath et al., 2014](https://doi.org/10.1016/B978-0-444-52512-3.00249-7)). The CSTV is commonly estimated using mathematical models fit to yield response data, including the Cate–Nelson split ([Cate & Nelson, 1971](https://doi.org/10.2136/sssaj1971.03615995003500040048x)), linear-plateau ([Anderson & Nelson, 1975](https://doi.org/10.2307/2529422)), quadratic plateau ([Bullock & Bullock, 1994](https://doi.org/10.2134/agronj1994.00021962008600010033x)), exponential models such as the Mitscherlich model ([Cerrato & Blackmer, 1990](https://doi.org/10.2134/agronj1990.00021962008200010030x)), and modified arcsine–log calibration curves ([Correndo et al., 2017](https://doi.org/10.1071/CP16444)), among others. In each case, yield serves as the primary response variable used to infer the soil phosphorus level at which fertilization ceases to be economically beneficial.

An alternative to explicitly fitting yield response curves is to classify soils into discrete fertility groups based on STP, such as very low, low, optimum, high, and very high phosphorus, with each category associated with an empirical probability of yield response to fertilization. For example, previous research has shown that the probability of observing a yield response to phosphorus fertilization is approximately 80 percent for very low STP, 65 percent for low, 25 percent for optimum, 5 percent for high, and less than 1 percent for very high STP ([Mallarino, 2023](https://crops.extension.iastate.edu/cropnews/2023/02/newly-updated-phosphorus-and-potassium-guidelines)). These probability-based fertility classes are widely used in extension recommendations because they provide intuitive guidance for decision-making. However, the number of categories and the boundaries between them are often defined using predefined thresholds or expert judgment rather than an explicit, data-driven statistical framework, which may limit their ability to capture complex patterns in yield response across diverse environments.

In this context, our objective is to compare unsupervised learning methods for detecting patterns in wheat yield responses to phosphorus fertilization across Kansas sites. Specifically, we compare k-means clustering ([Likas et al., 2003](https://doi.org/10.1016/S0031-3203(02)00060-2)), fuzzy c-means ([Bezdek et al., 1984](https://doi.org/10.1016/0098-3004(84)90020-7)), and archetype analysis ([Cutler & Breiman, 1994](https://doi.org/10.1080/00401706.1994.10485840)) to assess how consistently each method identifies meaningful groupings in soil test phosphorus and relative yield data. We then compare these data-driven groupings with CSTVs derived from commonly used calibration approaches. By linking the resulting clusters to expert agronomic interpretation, we aim to identify which methods provide the most informative and reliable guidance for phosphorus fertilizer management.
...
The dataset used in this study comes from Kansas winter wheat phosphorus fertilization trials conducted during the 2019 and 2020 growing seasons, resulting in a total of 24 site–years. Phosphorus was applied as monoammonium phosphate at four rates: 0, 45, 90, and 135 kg P ha$^{-1}$. Each fertilizer rate was applied to four replicated plots at each site–year, yielding a total of $24 \times 4 \times 4 = 384$ plot-level observations.

All trials were established using a randomized complete block design (RCBD). Within each site–year, all phosphorus treatments were randomly assigned within each replication. Wheat was managed according to standard agronomic practices for the region, including consistent seeding rates, planting dates, and pest management. The same or locally recommended winter wheat variety was used within each site–year to minimize within-site genetic variability, though varieties may have differed among locations to reflect regional adaptation.

The dataset includes grain yield and soil test phosphorus (STP) measurements for each replicated plot. Soil samples were collected from the 0–15 cm depth, with approximately 10 to 12 soil cores composited per plot. Soil test P was determined using the Mehlich-3 colorimetric extraction method.

## 2.2 Mixed-effects model for yield response

Grain yield response to phosphorus fertilization was evaluated using a linear mixed-effects model to quantify overall yield trends across environments. The treatment structure followed a one-way factorial design with phosphorus rate as the treatment factor, including levels of 0, 45, 90, and 135 kg P ha$^{-1}$. The experimental design used in this analysis exactly matches the field experimental design described in Section 2.1.

Statistical inference on treatment differences was conducted using a mixed-model approach in which phosphorus rate was treated as a fixed effect. Site–year was included as a random effect to account for environmental variability among locations and years, and replications nested within site–year were also treated as random effects.

The model was fitted using restricted maximum likelihood (REML) with the `lmer()` function from the `lme4` package in R. An analysis of variance (ANOVA) table for the fixed effect of phosphorus rate was obtained using Type III F-tests with the Kenward–Roger approximation for denominator degrees of freedom. Model assumptions were evaluated using simulation-based residual diagnostics from the `DHARMa` package. Estimated marginal means of grain yield for each phosphorus rate were obtained using the `emmeans` package and used to summarize the overall yield response to P across sites. All analyses were conducted in R (version 4.5.0), and the corresponding R code is provided in the Supplementary Material.

The linear mixed-effects model was defined as:
\[
y_{ijk} = \mu + \tau_i + s_j + b_{k(j)} + \varepsilon_{ijk},
\]
where $y_{ijk}$ denotes the grain yield observed at phosphorus rate $i$ in block $k$ of site–year $j$, $\mu$ is the overall mean, $\tau_i$ is the fixed effect of the $i$th phosphorus rate ($i = 1,\dots,4$), $s_j \sim \text{Normal}(0,\sigma_s^2)$ is the random effect of site–year $j$, $b_{k(j)} \sim \text{Normal}(0,\sigma_b^2)$ is the random effect of block $k$ nested within site–year $j$, and $\varepsilon_{ijk} \sim \text{Normal}(0,\sigma^2)$ is the residual error. All random effects were assumed to be independent.
...
In both CSTV models, the response variable was relative yield (RY). For each site–year, relative yield was defined as
\[
\text{RY} = \frac{Y_0}{\max(Y_0, \dots, Y_R)},
\]
where $Y_0$ is the estimated mean grain yield of the control treatment (0 kg P ha$^{-1}$) and $\max(Y_0, \dots, Y_R)$ is the maximum estimated mean yield observed across all phosphorus rates at that site–year.
...
Clustering is an unsupervised machine learning approach used to identify patterns in data by grouping observations with similar characteristics. In this study, we applied three clustering methods—k-means, fuzzy c-means, and archetype analysis—to derive data-driven groupings in soil test phosphorus and relative yield.
...
Archetype analysis was applied to a two-dimensional feature space consisting of soil test phosphorus and relative yield. Archetypes represent convex combinations of extreme observations in the data cloud and correspond to distinct soil P–response profiles. A three-archetype solution was selected to facilitate comparison with the $k = 3$ clustering approaches.
