...
X_resp <- df_features %>%
  dplyr::select(intercept, slope, STP,delta_y) %>%  
  drop_na() %>%
  scale() 
...
rss_k <- sapply(seq_along(k_vals), function(i) {
  rss(bestModel(as_step[[i]]))
})
...
best_index <- which.max(drops) + 1  
best_k    <- k_vals[best_index]

best_k

# Best model
final_arch <- bestModel(as_step[[best_index]])

# Coefficients (
alpha <- coef(final_arch)

# Assign each obs to the archetype with the largest weight
arch_cluster <- apply(alpha, 1, which.max)

pca_scores <- pca_scores %>%
  mutate(archetype = factor(arch_cluster))

arch_points <- as.data.frame(parameters(final_arch))
names(arch_points) <- c("PC1", "PC2")
arch_points$archetype <- factor(1:nrow(arch_points))

arch_plot <- ggplot() +
  # Points: observations colored by closest archetype
  geom_point(
    data = pca_scores,
    aes(x = PC1, y = PC2, color = archetype),
    size  = 3,
    alpha = 0.8
  ) +
  ...
  # Archetypes: big yellow diamonds with labels
  geom_point(
    data = arch_points,
    aes(x = PC1, y = PC2),
    shape = 23,
  ...
  geom_text(
    data = arch_points,
    aes(x = PC1, y = PC2, label = archetype),
    vjust = -1.2,
    size  = 5
  ) +
  geom_vline(xintercept = 0, linetype = "dashed", linewidth = 0.4) +
  ...
  theme_bw() +
  labs(
    x     = "PC1 (P responsiveness)",
    y     = "PC2 (Baseline yield at 0 P)",
    color = "Archetype",
    title = "Archetype Analysis in PCA Response Space"
  )
...
fuzzy_cluster <- apply(fcm_fit$membership, 1, which.max)

pca_scores <- pca_scores %>%
  mutate(
    fuzzy_cluster = factor(fuzzy_cluster),
    max_membership = apply(fcm_fit$membership, 1, max) 
  )


fcm_plot <- ggplot(pca_scores,
                   aes(x = PC1, y = PC2, color = fuzzy_cluster)) +
  geom_point(size = 3, alpha = 0.8) +
  scale_color_viridis_d()+
  geom_vline(xintercept = 0, linetype = "dashed", linewidth = 0.4) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.4) +
  ...
  labs(
    ...
    y     = "PC2 (Baseline yield at 0 P)",
    color = "Fuzzy cluster",
    title = "Fuzzy c-means clusters in PCA response space"
  )
